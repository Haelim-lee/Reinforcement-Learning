...
namespace Reinforcement_Learning
{
	public class SarsaManager
	{
		...
		public string ActionValueFunctionFilePath = "SarsaActionValueFunction.json";
		public string FunctionAccuracyFilePath = "SarsaActionValueFunctionAccuracy.csv";

		public SarsaManager()
		{
			// 상태 가치 함수
			ActionValueFunction = new Dictionary<int, Dictionary<int, float>>();
			// 동적 프로그래밍 결과와 비교한 구해진 가치 함수의 정확도를 저장하는 dictionary
		...
		public void ApplySarsa()
		{
			...
			while (keepUpdating)
			{
				...
				if (episodeCount % 1000 == 0) // 에피소드 1000개 수행할 때 마다 상태 출력
				{
					...
					{
						valueFunctionAccuracy = Utilities.EvaluateValueFunction(QFunctionType.SARSA);
				...
				if (episodeCount > 1000000) // 에피소드 100만개 처리 후 종료
				{
					if (isDPFunctionAvailable)
					{
						FunctionAccuracyForEpisodeCount.Add(episodeCount, Utilities.EvaluateValueFunction(QFunctionType.SARSA));
					}
					keepUpdating = false;
				}
			}
