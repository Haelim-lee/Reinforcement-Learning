		...
		public void ApplyQLearning()
		{
			...
			while (keepUpdating)
			{
				...
				if (episodeCount % 1000 == 0) // 에피소드 1000개 수행할 때 마다 상태 출력
				{
					...
					{
						valueFunctionAccuracy = Utilities.EvaluateValueFunction(QFunctionType.QLEARNING);
				...
				if (episodeCount > 1000000) // 에피소드 100만개 처리 후 종료
				{
					if (isDPFunctionAvailable)
					{
						FunctionAccuracyForEpisodeCount.Add(episodeCount, Utilities.EvaluateValueFunction(QFunctionType.QLEARNING));
					}
					keepUpdating = false;
				}
			}
